...
### Checking enrichment with scz DE
...
### PUll in scz data
load('/users/ajaffe/Lieber/Projects/RNAseq/SzControl_DE_paper/rdas/expressed_de_features.rda') #out
out_gene = as.data.frame(outStatsExprs[['Gene']][names(outStatsExprs[['Gene']])%in%TESTED_UNIV ])

############## Gene set Test with limma ############
limma::geneSetTest(which(rownames(out_gene)%in%INT_DE), out_gene[,'Adj_ba41_42_22.tstat'] )

int_p = sapply(colnames(out_gene)[grepl("tstat",colnames(out_gene))], function(x) {
limma::geneSetTest(which(rownames(out_gene)%in%INT_DE), abs(out_gene[,x]) ) })

prenatal_p = sapply(colnames(out_gene)[grepl("tstat",colnames(out_gene))], function(x) {
limma::geneSetTest(which(rownames(out_gene)%in%PRENATAL_DE), abs(out_gene[,x]) ) })

out_gene$interactionDE = ifelse(rownames(out_gene)%in%INT_DE, "Yes", "No")
out_gene$prenatalDE = ifelse(rownames(out_gene)%in%PRENATAL_DE, "Yes", "No")
out_gene$EnsemblGeneID = rownames(out_gene)
out = out_gene
...
#
mean_t <- dplyr::group_by(dat,Model, interactionDE) %>%
                 summarize(
